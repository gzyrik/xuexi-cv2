...
static cv::Mat Matting(const cv::Mat& nv12)
{
    ...
    InferenceEngine::Blob::Ptr yBlob = InferenceEngine::make_shared_blob<uint8_t>(yDesc, nv12.data);
...
void main()
{
    ...
    while (cv::waitKey(1) == -1 && camera.read(frame) ) {
        ...
        cv::Mat nv12; {
            ...
            const size_t H_2 = (H + 1) >> 1, W_2 = (W + 1) >> 1;
            cv::cvtColor(image, nv12, cv::COLOR_BGR2YUV_I420);
            uint8_t* u = nv12.data+(W*H), *v = u + W_2*H_2, *d=_uvBuf;
            for (int i = 0; i < W_2*H_2; ++i)
                *d++ = u[i], *d++ = v[i];
            memcpy(nv12.data+(W*H), _uvBuf,  W_2*H_2);
        }
